...
<div class="min-h-screen bg-gray-50">
    ...
    <div class="sticky top-0 z-50 bg-white border-b border-gray-200 px-4 py-3 shadow-sm">
        <div class="flex items-center justify-between">
            ...
            <h1 class="text-lg font-semibold text-gray-900">Add Product</h1>
    ...
    <!-- Mobile Form -->
    <form id="productForm" action="{{ route('admin.products.store') }}" method="POST" enctype="multipart/form-data" class="pb-24">
        @csrf
            ...
            <div class="bg-white px-4 py-4 space-y-5">
                ...
                <div>
                    ...
                    <textarea name="fragrance_notes" 
                              ...
                              autocomplete="off">{{ old('fragrance_notes') }}</textarea>
                        ...
                        @foreach(['men', 'women', 'unisex'] as $gender)
                            <label class="block">
                                <input type="radio" 
                                       ...
                                       value="{{ $gender }}" 
                                       class="sr-only peer"
                                       {{ old('gender', 'unisex') == $gender ? 'checked' : '' }}
                                       required>
        ...
        <div class="mb-6">
            ...
            <div class="bg-white px-4 py-4 space-y-5">
                ...
                <!-- Price Preview -->
                @if(old('price_100ml') || old('discount_100ml'))
                <div class="p-4 bg-gray-50 rounded-xl">
                    <div class="space-y-2">
                        <div class="flex items-center justify-between">
                            <span class="text-gray-600">100ml Original:</span>
                            <span class="text-gray-900 font-medium">${{ number_format(old('price_100ml', 0), 2) }}</span>
                        </div>
                        @if(old('discount_100ml') && old('discount_100ml') > 0)
                        <div class="flex items-center justify-between">
                            <span class="text-gray-600">100ml Discount:</span>
                            <span class="text-green-600 font-bold">${{ number_format(old('discount_100ml'), 2) }}</span>
                        </div>
                        <div class="pt-2 border-t border-gray-200">
                            <div class="flex items-center justify-between">
                                <span class="text-gray-600">You Save:</span>
                                <span class="text-red-600 font-bold">
                                    ${{ number_format(old('price_100ml', 0) - old('discount_100ml'), 2) }}
                                    @if(old('price_100ml', 0) > 0)
                                    <span class="text-sm">({{ number_format((old('price_100ml', 0) - old('discount_100ml')) / old('price_100ml', 0) * 100, 0) }}%)</span>
                                    @endif
                                </span>
                            </div>
                        </div>
                ...
                <div class="space-y-2">
                    ...
                    <label class="flex items-center p-3 border border-gray-300 rounded-xl active:bg-gray-50">
                        <input type="checkbox" 
                               ...
                               {{ in_array($fragrance->id, old('fragrances', [])) ? 'checked' : '' }}>
        ...
        <div class="mb-6">
            ...
            <div class="bg-white px-4 py-4">
                ...
                <div id="imageUploadArea" 
                    ...
                    <h4 class="font-medium text-gray-900 mb-1">Tap to Upload Image</h4>
                    ...
                    <p class="text-xs text-gray-400 mt-3">Supported: JPEG, PNG, GIF</p>
                </div>
                @error('main_image')
                    <p class="mt-1 text-sm text-red-600">{{ $message }}</p>
                @enderror
                    ...
                    <div class="p-4 border border-gray-200 rounded-xl">
                        <div class="flex items-center">
                            <div class="w-20 h-20 rounded-lg bg-gray-100 overflow-hidden flex-shrink-0">
                                <img id="previewImage" class="w-full h-full object-cover" alt="Product preview">
                            </div>
        ...
        <div class="mb-6">
                ...
                <div class="flex items-center justify-between">
                    <div class="flex items-center">
                        ...
                        <div>
                            ...
                            <p class="text-xs text-gray-500">Optional - Add other sizes (50ml, 200ml, etc.)</p>
                        </div>
                    </div>
                    <button type="button" 
                            id="add-size" 
            ...
            <div class="bg-white px-4 py-4">
                ...
                <!-- Empty State -->
                <div id="no-sizes" class="text-center py-8">
                    <div class="w-14 h-14 mx-auto mb-3 rounded-full bg-gray-100 flex items-center justify-center">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function addSizeField(prefillData = null) {
        if (noSizes) noSizes.style.display = 'none';
    ...
    form.addEventListener('submit', function(e) {
        ...
        if (!imageInput.files.length) {
            isValid = false;
            document.getElementById('imageUploadArea').classList.add('border-red-500');
        }
        ...
        if (!isValid) {
            ...
            errorDiv.innerHTML = 'Please fill in all required fields and upload an image';
